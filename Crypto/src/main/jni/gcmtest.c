...
/*
    if (!(length & 1)) { 
        fprintf(stderr,
            "Cannot have odd number of characters in input: %d passed:%d\n",
            sizeof(*hex),length);
	return -1;
    }
*/
...
void hexArray(char *hexString,uchar *ptr, size_t length)          
{         
    //for statment to print values using array             
    size_t i = 0;
    //int len = 0;
    //uchar *intermediate;
    //intermediate =malloc(2);
    sprintf(hexString,"");
    for( ; i<length; ++i )
    {
	//printf("S:%s\n",hexString);
	sprintf(hexString,"%s%02x",hexString,ptr[i]);
	//strcat(hexString,intermediate);
...
int verify_gcm( uchar *vd )
{
	...
	char outKey[key_len*2],outIV[iv_len*2],outAAD[aad_len*2],outPT[pt_len*2],outCT[ct_len*2],outTag[tag_len*2];
	printArray(key,key_len);
	hexArray((char *)&outKey,key,key_len);
	hexArray((char *)&outIV,iv,iv_len);
	hexArray((char *)&outAAD,aad,aad_len);
	hexArray((char *)&outPT,pt,pt_len);
	hexArray((char *)&outCT,ct,ct_len);
	hexArray((char *)&outTag,tag,tag_len);
	...
	/*
	printf("test %d\n",i);
	if(RecordType ==1) 	printf("Encrypt Auth %d\n",i);
	if(RecordType ==2) 	printf("Decrypt Auth Correct %d\n",i);
	if(RecordType ==3) 	printf("Decrypt Auth INCorrect %d\n",i);
	printf( "Key Len:\"%d\"\n", key_len );
	//printf( "Key:\"%X\"\n", key_len );
	printArray(key,key_len);
        key2 = (uchar *)malloc(key_len);
	printf("ucahr size:%d\n",sizeof(uchar));
	hexdec2(key2,(uchar *)"e17c5ca66629bad686d9835103f8cb2843c0980bc6ef2947ea5ccec384a25599",32);
	printArray(key2,key_len);

	printf( "IV Len:\"%d\"\n", iv_len );
	printArray(iv,iv_len);

	printf( "AAD Len:\"%d\"\n", aad_len );
	printArray(aad,aad_len);

	printf( "PT Len:\"%d\"\n", pt_len );
	printArray(pt,pt_len);

	printf( "CT Len:\"%d\"\n", ct_len );
	printArray(ct,ct_len);

	printf( "Tag:\"%d\"\n", tag_len );
	printArray(tag,tag_len);
	*/
